...
| WinRAR（`Rar.exe`/`WinRAR.exe`，`server.py:1524`） | 打包下载输出 `.rar` | 可选（缺失自动降级 `.zip`） |
| UnRAR.exe（`server.py:1542`） | rar 包在线解压列表/单条目下载 | 可选（缺失时 rar 解压返回错误） |
...
`python server/server.py --serve [根目录|auto] --port <端口> --token <token>`（参数解析见 `server.py:3867-3878`）。启动后打印 HTTPS 访问 URL（IPv6 + 127.0.0.1）、HTTP 免证书 URL（**与 HTTPS 同一端口**）、根目录与打包格式，Ctrl+C 停止。`--serve` 缺省值 `auto` = 整机所有固定磁盘；也可传单个根目录（如 `D:\资料`）。
...
| 工具 | 参数 | 作用 |
|---|---|---|
| `drive_start` | `root`（默认 "auto"）、`port`（默认 8443） | 启动服务，返回 URL/打包格式（`server.py:3730`） |
| `drive_pin` | `paths`（绝对路径数组） | 把文件/目录置顶到网页显著位置（`server.py:3754`） |
| `drive_status` | 无 | 查询运行状态/根目录/URL（`server.py:3807`） |
| `drive_stop` | 无 | 停止服务并清理防火墙规则（`server.py:3827`） |

## 5. 服务端架构与模块（server.py）

### 5.1 核心类

- **`_DriveServer(ThreadingHTTPServer)`**（`server.py:1333`）：`AF_INET6` 地址族 + 关闭 `IPV6_V6ONLY`（`server_bind`，`server.py:1378-1385`）实现**双栈监听**——`[::]` 同时接受 IPv4-mapped 连接，手机/本机 IPv4 均可达；`daemon_threads=True`；`allow_reuse_address = False`（`server.py:1339`，Windows 下禁止同端口多实例并存，端口被占时 bind 直接抛错退出）。持有 `roots`（允许根目录列表）、`token`、`pinned`（置顶列表）、`ssl_context`。**重写 `get_request` 做首字节嗅探**（`server.py:1348-1376`）：accept 后 `recv(1, MSG_PEEK)` 窥探首字节，`0x16`（TLS ClientHello）则 `wrap_socket` 走 HTTPS（握手设 5s 超时防半截 ClientHello 阻塞），否则按明文 HTTP 处理——单端口 8443 同时服务 HTTPS 与 HTTP。
- **`_DriveHandler(BaseHTTPRequestHandler)`**（`server.py:118`）：`protocol_version = "HTTP/1.1"`（便于 `<video>` 复用连接做 Range 分片）。核心方法：
  ...
  - `do_GET`（`server.py:986`）、`do_POST`（`server.py:1278`）：路由表，见第 6 节

### 5.2 关键独立函数/模块

- **磁盘枚举** `_fixed_drives`（`server.py:1477`）：`GetLogicalDrives` + `GetDriveTypeW==DRIVE_FIXED` 枚举固定磁盘。
- **目录列表** `_list_dir`（`server.py:1492`）：scandir，目录在前按名排序；系统噪声文件（DumpStack.log/hiberfil.sys/pagefile.sys/swapfile.sys，`_SYSTEM_NOISE` `server.py:1572-1573`）标记 `locked=True` 前端灰显不可下载；权限错误返回 None。
- **视频元数据** `_video_details`（`server.py:1736`）：ffprobe `-show_format -show_streams` 提取 tag/时长/码率/分辨率/声道等，进程内缓存 key=(realpath,size,mtime)（`server.py:1620`），任何失败静默返回 None；`_parse_comment_meta`（`server.py:1623`）把 comment 标签解析为结构化键值（上传/观看/点赞/标签/作者/类型），`_video_meta`（`server.py:1688`）构建前端详情面板数据。
- **meta=1 批量元数据**（见第 5.4 节）：`_meta_kind`（`server.py:1872`）扩展名→ 分类/mime；`_video_meta_cached`（`server.py:1894`）仅从进程内缓存读视频内容元数据（零 IO）；`_probe_video_meta`（`server.py:1930`）后台并发探测；`_augment_entries_meta`（`server.py:1955`）给每个 entry 附加 meta 字段。
- **缩略图** `_thumb_path`（`server.py:2098`）：ffmpeg 抽帧第 3 秒（失败回退 0 秒），`scale=360:-2`，缓存 `static/thumbs/<md5(realpath|size|mtime)>.jpg`。
- **打包**：`_find_winrar`（`server.py:1524`）/ `_find_unrar`（`server.py:1542`）多路径探测（支持 `WINRAR_PATH`/`UNRAR_PATH` 环境变量）；`_stream_archive`（`server.py:3340`）WinRAR 出 `.rar` 否则 zipfile 出 `.zip`；`_stream_archive_virtual`（`server.py:3397`）虚拟分享打包（同名复用文件写 `.lnk` 文本条目）。
- **解压**：`_unpack_list`（`server.py:3226`）zip 用 zipfile、rar 用 `UnRAR lb -p-`（GBK 输出）；`_unpack_download`（`server.py:3291`）zip 流式、rar 由 `UnRAR p` 输出。
- **防火墙**：`_add_firewall_rule`（`server.py:3610`）netsh 添加入站规则 `TransferMCP-<port>`，失败返回含修复命令的提示；`_remove_firewall_rule`（`server.py:3634`）停止时删除。
- **URL 生成**：`_public_ipv6`（`server.py:3583`）找非链路本地非 ULA 的 IPv6；`_urls`（`server.py:3597`）HTTPS 链接、`_urls_http`（`server.py:3603`）HTTP 免证书链接（**与 HTTPS 同一端口**）。
- **证书**：`_ensure_cert`（`server.py:3528`）不存在/损坏/过期/SAN 无 IP 时生成自签名证书（CN/SAN=`transfer.local`，RSA-2048，有效期 7 天，SHA256，SAN 含本机全部 IP，`_cert_san` `server.py:3485`）；`_cert_p12_bytes`（`server.py:3566`）打包 PKCS#12（密码 `1234`，`server.py:3563`，小米等 Android 装 CA 必需）。
- **MCP 工具**：`drive_start/drive_pin/drive_status/drive_stop`（`server.py:3730-3834`），`_under`（`server.py:3837`）做根目录包含校验。

### 5.3 MSE 转码链路（`server.py:2376-2760`）

- **会话模型**：`_trans_sessions` 全局字典，key=`(realpath, q, start_sec, persist)`；每个会话 = 一个常驻 ffmpeg 进程（`_trans_encode_thread`，`server.py:2411`）从 `start_sec` 起**连续**输出 fragmented MP4 到内存缓冲，前端按 offset 顺序拉取拼接（保证 moof 序列号连续）。
- **四档画质** `_TRANS_KEYS`/`_TRANS_QUALITIES`（`server.py:84-85`）：`original`（`-c copy` 快速 remux 不重编码，`_trans_remux_args` `server.py:2403`）/ `high`(1920) / `medium`(1280) / `low`(854)，转码参数为 `libx264 High 5.1 ultrafast/zerolatency + aac 128k + crf27 + keyint 60`（`_trans_args` `server.py:2376`）。
- **会话生命周期**：`_trans_ensure_session`（`server.py:2491`）创建/复用；`persist=False`（MSE 播放与分享页）不落盘完整缓存；`_trans_consume`（`server.py:2586`）按 offset 取数据带 5s 等待上限；`_trans_sweep_loop`（`server.py:2618`）后台线程每 5s 按 `last_client` 空闲超时 30s 回收会话并杀进程。
- **编码器能力探测**：`_ffmpeg_capabilities`（`server.py:2181`）惰性执行 `ffmpeg -encoders` 检查 libx264/aac，防止精简版 ffmpeg 静默失败。
- **MSE codec 契约**：`/api/vinfo` 的 `mseMime`（`_vinfo_mime` `server.py:2349`）——转码档固定 `avc1.640033`（High 5.1）+ 有条件 `mp4a.40.2`；原画档按源编码换算 `avc1.PPCCLL`（`_avc1_from_profile` `server.py:2288`）/ `hvc1`（`_hvc1_from_profile` `server.py:2305`）。
- **原画缓存下载**（`server.py:2762-2927`）：`cache=1` 时按 Range 边播边写"稀疏文件 + 区间集合"（`original.cache` + `original.cache.json`），per-digest 锁串行化合并（`_stream_cached` `server.py:2824`）。
- **缓存容量治理**：`_cache_sweep_loop`（`server.py:2686`）每 600s 执行 `_cache_sweep_once`（`server.py:2641`），transcache 超 2GB 按 mtime 从旧到新删，跳过活动会话的 cfile 与 `.tmp` 文件。

### 5.4 meta=1 批量元数据（侧边栏数据源）

`api/list?meta=1` 为每个 entry 附加 meta 字段（`_augment_entries_meta`，`server.py:1955`）：

- **目录** → `{"kind": "dir"}`
- **文件** → `{"kind": 分类, "mime": 类型}`，分类集合固定 video/audio/image/text/code/archive/exe/lnk/other（`_meta_kind`，`server.py:1872`），按扩展名判定，文本/代码无需读内容
- **视频** → 额外附加 `duration`（秒）/`width`/`height`，以及内容性字段 `title/author/type/tags/notes`——全部取自 `_video_details_cache` 进程内缓存（`_video_meta_cached`，`server.py:1894`），**零 IO、绝不运行 ffprobe**
- **未命中缓存的视频** → 进探测队列由 `_probe_video_meta`（`server.py:1930`）后台并发探测：8 个 worker、单次总预算 3s（`_VIDEO_PROBE_WORKERS`/`_VIDEO_PROBE_BUDGET_SEC`，`server.py:1821-1822`）、多请求互斥锁，**到点立即返回，任何情况下不阻塞列表响应**；预算外未完成任务由 executor 后台线程继续，成功即写缓存，下次 meta=1 直接命中

### 5.5 字幕 / 识别 / 预览

- `_subtitle_vtt`（`server.py:2993`）：同目录旁挂字幕（`.srt/.vtt/.ass/.ssa`，`server.py:2931`）优先，其次 ffmpeg 提取内嵌字幕流转 WebVTT；`_srt_to_vtt`（`server.py:2952`）/ `_ass_to_vtt`（`server.py:2964`）格式转换。
- `_asr_transcribe`（`server.py:3052`）：faster_whisper `small` 模型（CPU int8）单例复用 + per-path 锁 + 结果缓存 `<digest>.asr.<lang>.vtt`；语言 ja/en/zh。
- `_vthumbstrip_gen`（`server.py:3134`）：整段横向缩略图条（≤20 帧 tile）缓存 `<digest>.strip.jpg`；`_vframe_gen`（`server.py:3173`）：任意秒单帧预览缓存 `<digest>.f<秒>.jpg`。

### 5.6 后台线程（全部 daemon）

| 线程 | 启动处 | 周期 | 职责 |
|---|---|---|---|
| HTTPS/HTTP 服务 `serve_forever` | `_start` `server.py:3665` | 常驻 | 8443（单端口，首字节嗅探自动识别 TLS/明文） |
| `_trans_sweep_loop` | `_start` `server.py:3668` | 5s | 回收空闲转码会话 |
| `_cache_sweep_loop` | `_start` `server.py:3670` | 600s | 转码缓存磁盘治理 |

## 6. 完整路由 / API 清单

### 6.1 主站路由（前缀 `/transfer/`，token 可变）

**token 前缀保护机制**（`do_GET` `server.py:992-995`）：除 `/s/` 分享路由外，所有主站请求必须满足 `path == "/<token>"` 或以 `"/<token>/"` 开头，否则返回 403 `{"error":"无效 token"}`。**访问 `/transfer`（无尾部斜杠）会 301 重定向到 `/transfer/`**（`server.py:998-1003`）——否则页面内相对路径 `static/...` 会被浏览器解析成 `/static/...`（丢失 token 前缀）导致 403。`do_POST` 同理要求 `/<token>/` 前缀（`server.py:1283-1286`）。
...
| 子路径 | 说明 |
|---|---|
| `""`（空） | 分享页 = 主站模板；不存在 404 纯文本；**过期返回 200 过期提示页**（`server.py:965`，不在此删除以便稳定重复访问） |
...
| `static/*` | 复用主站静态白名单 |
| `api/pin` / `api/upload` / `api/cert` / `api/certp12` / `api/share` | **分享模式禁用，返回 403**（`server.py:960-962`） |

过期分享其余 API 返回 410 `{"error":"链接已过期"}`（`server.py:646`）；创建新分享时顺带清理过期 token（`server.py:1141-1144`）。
...
入口 `init()`（`app.js:2229`）：分享模式隐藏主站 UI（磁盘/置顶/打包/上传，`hideMainUi` `app.js:2220`）只读浏览分享根；主站模式加载磁盘/置顶并**从 localStorage 恢复上次目录与视图**。

| 功能 | 说明 | 位置 |
|---|---|---|
| 磁盘标签页 | 顶部横向滚动胶囊按钮，点击切换磁盘根 | `renderDriveTabs` `app.js:2295` |
| 面包屑 + 后退/前进 | 分段导航；栈式前进后退（`_pushNav` `app.js:2411`，按钮 `app.js:2422-2423`） | `renderBreadcrumb` `app.js:2341` |
| 列表/网格视图 | 网格视图视频显示 ffmpeg 缩略图封面（失败回退图标）；视图偏好持久化 | `listItem` `app.js:3118` / `gridItem` `app.js:3154` |
| 置顶（星标） | 列表/网格行内 ☆ 切换；置顶卡片区提供下载/分享/取消；"打包 .rar 下载"与"全部分享"按钮 | `renderPinned` `app.js:2314` |
| 上传 | 多文件顺序上传 + 进度条；分享模式无此按钮 | `app.js:3259-3284` |
...
| 文本/Markdown 预览 | 内置 Markdown 渲染器（转义后套格式防 XSS，`renderMarkdown` `app.js:1954`）；代码高亮（≤200KB 用 hljs，超大分片渲染）；大文本分片渲染 + 取消；400KB 截断提示 | `showText` `app.js:2045` |
| CSV 预览 | 简易引号感知解析器；前 300KB / 2000 行上限 | `showCsv` `app.js:3365` |
| PDF 预览 | iframe 内联 + 新窗口兜底链接 | `showPdf` `app.js:3430` |
| 压缩包解压预览 | 条目列表（≤5000 条），点击条目直接下载 | `showUnpack` `app.js:2146` |
| .lnk 快捷方式 | 显示目标、进入目标目录/下载原文件（越界时"返回上级"靠 navStack 回退链兜底，`app.js:3087-3095`） | `showLnk` `app.js:3455` |
| 分享链接 | 单文件/目录（`showShareDialog` `app.js:3508`）、全部置顶（`showShareManyDialog` `app.js:3572`）、分享页内二次分享（`showSubShareDialog` `app.js:3633`）；有效期 1/24/72/168 小时；复制/打开链接 | |
| 状态恢复 | localStorage `drive.cur/drive.root/drive.view/drive.modal`；弹窗加入浏览器历史（手机返回键关弹窗不退出）；分享模式不读写主站键 | `saveDriveState` `app.js:3321` |
| 错误处理 | 无权限目录显示"返回上级"按钮；token 校验失败 403 JSON 提示；虚拟分享根为空串时靠 navStack/activeRoot 兜底 | `showAlert` `app.js:930` |

### 7.1 侧边栏（视图与筛选）

右侧滑出面板（HTML：`index.html:234-292`，CSS `index.html:128-151`），打开/关闭 `openSidebar`/`closeSidebar`（`app.js:2992-3003`），关闭不改变筛选状态。列表加载统一带 `meta=1`（`loadList` `app.js:3076-3082`），侧边栏所有操作**基于 `currentEntries` 前端过滤重渲染，不重新请求**。

| 区块 | 说明 | 位置 |
|---|---|---|
| 搜索 | 输入即过滤，**150ms 防抖**（`app.js:3013-3018`）；先 `normalizeCJK` 简繁归一化再小写（`normSearch` `app.js:2449`） | `index.html:243-251` |
...
| 类型筛选（多选） | 视频/文本/编程/软件包/压缩包/快捷方式/目录 + 自输入后缀（逗号分隔，如 `md,txt,json`）；类型判定基于 meta.kind（`entryKindMatch` `app.js:2430`），硬筛选含后缀 OR 逻辑（`applyHardFilters` `app.js:2794`） | `index.html:267-279`、`app.js:3031-3035` |
| 排序 | 修改时间/名称（本地化 zh-Hans-CN）/大小，**双向切换**：再次点击当前排序项即反转方向（原生 select 重复选择不触发 change，用 **mousedown+click 协作**手动反转，`app.js:3040-3059`）；默认方向 mtime 新→旧 / name 升序 / size 大→小（`DEFAULT_SORT_DIR` `app.js:2835`）；目录永远排前（`entryCompare` `app.js:2856`）；option 文本随方向动态更新（`updateSortLabel` `app.js:2842`） | `index.html:280-287` |
| 重置 | 一键清空筛选/搜索/恢复默认排序 | `app.js:3061-3071` |

### 7.2 搜索关键词语法（parseQuery）

查询统一由 `parseQuery`（`app.js:2812-2819`）解析为 `[{and:[...]},...]` 组结构，精确与模糊两阶段共用：
...
精确阶段（`filterEntries` `app.js:2823`）对归一化后的可搜索文本做子串匹配；模糊阶段（`fuzzyMatchTexts` `app.js:2783`）对同一组结构做概率判定。

### 7.3 模糊匹配（概率模型 + 两阶段渲染）

**概率匹配模型**（`app.js:2657-2790`，纯函数可单测）：
...
判定 `final = max(s_lcs, s_edit, s_perm) ≥ 0.8` 且多集覆盖率 ≥ 0.8（`fuzzyMatchKw` `app.js:2765`），阶段短路：预筛 → 字符硬约束 → LCS 达标 → 滑窗编辑达标 → 乱序加权。

**两阶段渲染**（`renderEntries` `app.js:2874`）：
...
| 数据 | 位置 | 格式 / 说明 |
|---|---|---|
| 自签名证书 | `~/.transfer-mcp/server.crt` + `server.key` | PEM，RSA-2048，7 天有效期，CN/SAN=`transfer.local`、SAN 含本机全部 IP；缺失/损坏/过期/SAN 无 IP 时自动重建（`server.py:57-61, 3482, 3528`） |
| 分享记录 | `~/.transfer-mcp/shares.json` | `{token: {root, is_dir, expires_at, created_at, name, [files, virtual, nodes, parent]}}`；写临时文件再 rename 防损坏（`_save_shares` `server.py:3210`）；**模块加载即恢复**（跨进程重启链接仍有效，`server.py:3223`） |
...
| 视频缩略图 | `static/thumbs/<md5(realpath\|size\|mtime)>.jpg` | ffmpeg 抽帧，文件变化自动失效，已在 .gitignore 排除（`server.py:2098-2135`） |
| 视频元数据缓存 | 进程内存 `_video_details_cache` | key=(realpath,size,mtime)，ffprobe 失败也缓存 None；meta=1 的深度查找/推荐标签均只读此缓存（`server.py:1620`） |
| 置顶列表 pinned | 仅内存（`_DriveServer.pinned` / `_state["pinned"]`） | **重启即丢失**；由 MCP `drive_pin` 或网页星标维护 |
| 推荐标签缓存 | 前端内存 `_tagCache`（Map: path → scores） | per-path 缓存避免重复扫描；目录切换自动失效重扫（`app.js:2550, 3113`） |
...
| `_VIDEO_PROBE_WORKERS` / `_VIDEO_PROBE_BUDGET_SEC` | 8 / 3.0（meta=1 视频探测并发上限与总预算） | `server.py:1821-1822` |
| 分享有效期选项 | 1 / 24 / 72 / 168 小时 | `server.py:97` |
| 证书有效期 | 7 天（前后 5 分钟缓冲） | `server.py:3541-3542` |
| `CERT_P12_PASSWORD` | "1234" | `server.py:3563` |
...
| 转码输出 | H.264 High 5.1 (avc1.640033) + aac 128k，crf 27，keyint 60，fMP4 | `server.py:2376-2400` |
| 防火墙规则名 | `TransferMCP-<port>` | `server.py:3611` |
| MCP 首选协议 | 2025-06-18 | `mcp_stdio.py:22` |
| 搜索防抖 | 150ms | `app.js:3017` |
...
- **token 前缀保护**：所有主站 API 必须带 `/transfer/` 前缀（`server.py:992-995`），否则 403；`/transfer` 无斜杠会 301 到 `/transfer/`（`server.py:998-1003`）。分享路由 `/s/` 独立于主 token，二者互不暴露。
...
- **firewall 规则**：添加入站规则需要管理员权限，失败时启动输出会给出手工执行命令（`server.py:3610-3631`）。
...
- **Windows 编码**：Rar.exe/UnRAR.exe 输出按 cp936 解码（`server.py:3258/3365`）；MCP stdio 强制 UTF-8（`mcp_stdio.py:182-186`）；前端上传中文文件名做 latin-1→utf-8 修正（`server.py:1309-1313`）。
- **大文件预览**：`/api/read` 默认只读前 1MB（前端再截断 400KB 渲染、CSV 300KB/2000 行、压缩包列表 5000 条），完整内容需下载。
- **深度查找 / 推荐标签依赖视频元数据探测**：元数据源于 ffprobe 的 tag/comment（`_parse_comment_meta` `server.py:1623`），只有视频文件且已被探测进缓存才有内容字段；meta=1 探测受 3s 预算限制，**超大目录首次浏览时部分视频可能暂无元数据**（深度查找/标签推荐会漏），重新打开目录（缓存已填充）即可补全。
...
- **单实例约束**：`_DriveServer.allow_reuse_address = False`（`server.py:1339`）+ 启动脚本的残留实例清理/端口检查（`启动网盘.bat:13-25`），杜绝"新旧实例共同监听一个端口、请求随机分发"的问题。